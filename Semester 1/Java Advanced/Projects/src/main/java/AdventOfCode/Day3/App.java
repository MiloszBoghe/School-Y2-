...
public class App {
    public static void main(String[] args) {
        ...
        Map<Point2D, Integer> points = getPoints(wire1);
        Map<Point2D, Integer> points2 = getPoints(wire2);

        Point2D center = new Point2D.Double(0, 0);
        Map<Point2D, Integer> intersections = getIntersections(points, points2);
        int min = getMinDistance(intersections);
        System.out.println(min);
    }

    public static Map<Point2D, Integer> getPoints(String wire) {
        ...
        for (String s : commands) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                length += 1;
                points.put(point, length);
            }
        }
        return points;
    }

    private static Map<Point2D, Integer> getIntersections(Map<Point2D, Integer> points, Map<Point2D, Integer> points2) {
        Map<Point2D, Integer> intersections = new HashMap<>();
        ...
        for (Map.Entry me : points.entrySet()) {
            if (points2.containsKey(me.getKey())) {
                ...
                length += (int) me.getValue();
                length += points2.get(me.getKey());
                if (length < minLength) {
                    minLength = length;
                }
            }
    ...
    public static int getMinDistance(Map<Point2D, Integer> intersections) {
        Point2D center = new Point2D.Double(0, 0);
        int min = Integer.MAX_VALUE;
        for (Map.Entry me : intersections.entrySet()) {
            Point2D point = (Point2D) me.getKey();
            int steps = (int) me.getValue();
    ...
    public static int getDistance(Point2D center, Point2D point) {
        return (int) (Math.abs(center.getX() - point.getX()) + Math.abs(center.getY() - point.getY()));
    }
}
